...
public:
    ...
    ros::Publisher pubRbfResidualStats;
    ros::Publisher pubLidarZOffset;
    ...
    cv::Mat matP;
    std::unique_ptr<lio_sam_rbf::RbfCudaGradientInterface> rbfGradientInterface;
    std::vector<lio_sam_rbf::RbfLmConstraint> rbfLmConstraints;
    double lidarZOffsetAdaptive = 0.0;
    double residualEma = 0.0;
    bool residualEmaInitialized = false;
    double lastRbfResidualMean = 0.0;
    double lastRbfResidualMeanAbs = 0.0;
    bool lastRbfConstraintUsed = false;
    ros::Time lastResidualPubTime;
    ...
    mapOptimization()
    {
        ...
        pubRbfResidualStats   = nh.advertise<std_msgs::Float64MultiArray>("lio_sam/rbf/residual_stats", 10);
        pubLidarZOffset       = nh.advertise<std_msgs::Float64>("lio_sam/lidar_calib/z_offset", 10);
        ...
        ROS_INFO_STREAM("[lio_sam] lidar z auto-calib: " << (enableLidarZCalibration ? "enabled" : "disabled")
                        ...
                        << ", range=[" << lidarZMin << ", " << lidarZMax << "]"
                        << ", lr=" << lidarZLearningRate);

        // Initialize TUM trajectory file
        // Enable saving if path is not empty
        if (!saveTUMTrajectoryPath.empty())
        {
            saveTUMTrajectory = true;
            tumTrajectoryFile.open(saveTUMTrajectoryPath, std::ios::out);
            if (tumTrajectoryFile.is_open())
            {
                ROS_INFO("TUM trajectory will be saved to: %s", saveTUMTrajectoryPath.c_str());
            }
            else
            {
                ROS_WARN("Failed to open TUM trajectory file: %s", saveTUMTrajectoryPath.c_str());
    ...
    void setRbfGradientInterface(std::unique_ptr<lio_sam_rbf::RbfCudaGradientInterface> interfacePtr)
    {
        if (interfacePtr)
            rbfGradientInterface = std::move(interfacePtr);
    }
    ...
    void updateRbfLmConstraints()
    {
        ...
        for (int i = 0; i < surfCount && static_cast<int>(request.points_lidar.size()) < takeCount; ++i)
        {
            const auto& p = laserCloudSurfLastDS->points[i];
            request.points_lidar.emplace_back(p.x, p.y, p.z);
        }

        for (int i = 0; i < cornerCount && static_cast<int>(request.points_lidar.size()) < takeCount; ++i)
        {
        ...
        for (int i = 0; i < surfMapCount && static_cast<int>(request.rbf_nodes.size()) < takeNodeCount; ++i)
        {
            const auto& p = laserCloudSurfFromMapDS->points[i];
            request.rbf_nodes.emplace_back(p.x, p.y, p.z);
        }
        ...
        const float weightScale = static_cast<float>(rbfConstraintWeight);
        for (auto& c : rbfLmConstraints)
            c.weight *= weightScale;

        // Publish residual statistics and adapt lidar Z offset from manifold residuals.
        if (!rbfLmConstraints.empty())
        {
            double sum = 0.0;
            double sumAbs = 0.0;
            double sumR1 = 0.0;
            ...
            for (const auto& c : rbfLmConstraints)
            {
                ...
                sumAbs += std::abs(static_cast<double>(c.residual));
            }

            const double mean = sum / static_cast<double>(rbfLmConstraints.size());
            ...
            {
                residualEma = mean;
                residualEmaInitialized = true;
            }
            else
            {
                const double alpha = std::min(1.0, std::max(0.0, lidarResidualEmaAlpha));
                residualEma = alpha * mean + (1.0 - alpha) * residualEma;
            }

            if (enableLidarZCalibration && static_cast<int>(rbfLmConstraints.size()) >= lidarResidualMinConstraintNum)
            {
                double update = 0.0;
                if (std::abs(residualEma) > lidarResidualDeadband)
                    update = -lidarZLearningRate * residualEma;
                lidarZOffsetAdaptive = std::min(lidarZMax, std::max(lidarZMin, lidarZOffsetAdaptive + update));
            }
            ...
            if ((now - lastResidualPubTime).toSec() > 0.05)
            {
                ...
                std_msgs::Float64 zOffsetMsg;
                zOffsetMsg.data = lidarZOffsetAdaptive;
                pubLidarZOffset.publish(zOffsetMsg);
                lastResidualPubTime = now;
            }
        }
        else
        {
            lastRbfResidualMean = 0.0;
            lastRbfResidualMeanAbs = 0.0;
        }
    }
    ...
    bool LMOptimization(int iterCount)
    {
        ...
        int rbfConstraintNum = static_cast<int>(rbfLmConstraints.size());
        int totalConstraintNum = laserCloudSelNum + rbfConstraintNum;
        lastRbfConstraintUsed = (rbfConstraintNum > 0);
        ROS_INFO_THROTTLE(1.0,
            "[lio_sam][rbf-lm] used=%s constraints=%d mean=%.5f mean_abs=%.5f",
            lastRbfConstraintUsed ? "true" : "false",
            rbfConstraintNum,
            lastRbfResidualMean,
            lastRbfResidualMeanAbs);

        cv::Mat matA(totalConstraintNum, 6, CV_32F, cv::Scalar::all(0));
        cv::Mat matAt(6, totalConstraintNum, CV_32F, cv::Scalar::all(0));
        cv::Mat matAtA(6, 6, CV_32F, cv::Scalar::all(0));
        cv::Mat matB(totalConstraintNum, 1, CV_32F, cv::Scalar::all(0));
        ...
        for (int i = 0; i < rbfConstraintNum; ++i)
        {
            int row = laserCloudSelNum + i;
            const auto& c = rbfLmConstraints[i];
            for (int j = 0; j < 6; ++j)
                matA.at<float>(row, j) = c.jacobian[j] * c.weight;
            matB.at<float>(row, 0) = -c.residual * c.weight;
        }
    ...
    void saveTUMTrajectoryToFile()
    {
        if (!saveTUMTrajectory || !tumTrajectoryFile.is_open())
            return;

        std::lock_guard<std::mutex> lock(tumFileMutex);
        
        // TUM格式: timestamp x y z qx qy qz qw
        double timestamp = timeLaserInfoCur;
        ...
        tumTrajectoryFile << std::fixed << std::setprecision(9) << timestamp << " "
