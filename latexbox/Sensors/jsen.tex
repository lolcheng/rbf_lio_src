...
The resulting smooth terrain manifold enables ``soft constraints" that regularize the odometry optimization and mitigates the $z$-axis pose drift under abrupt elevation changes during robot's maneuver. To ensure real-time performance, the RBF-related operations are accelerated on the GPU. Experiments on unstructured terrains demonstrate that the proposed method improves localization accuracy and map consistency, especially in scenarios with abrupt elevation changes and sparse geometric features.
\end{abstract}
...
\yzl{Next, we incorporate the manifold residuals into the feature-based scan-matching objective. Following the LIO-SAM front-end~\cite{liosam}, the IMU pre-integration module provides the initial guess for each LiDAR frame, while point-to-line and point-to-plane residuals form the original scan-matching cost:}
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $F^e_k$ and $F^p_k$ denote the edge and planar feature sets, and $d_e(\cdot)$ and $d_p(\cdot)$ are the corresponding point-to-line and point-to-plane residuals.
...
\yzl{Experiments are conducted on a Limx Tron1 legged-wheel robot operating in off-road locomotion mode. The robot is equipped with an inverted Mid360 LiDAR with its built-in IMU, a GPS+RTK module, and joint sensors, as shown in Fig.~\ref{fig:robot-setting}. The Mid360 provides 10 Hz point clouds and 200 Hz IMU measurements, the GPS+RTK module provides 10 Hz position references, and the joint sensors provide 500 Hz joint-angle measurements. The inverted LiDAR configuration increases the density of ground observations for terrain reconstruction, but it is a sensing configuration rather than a prerequisite of the proposed formulation. All experiments are run on a laptop with an Intel i5-12490F CPU, 32 GB RAM, and an NVIDIA RTX 3070 GPU.}

\subsection{The Dataset}

\begin{figure}[!htpb]
  \centering
  \includegraphics[width=1\linewidth]{figure/map_env/dataset_new.png}
    \caption{Dataset scenarios: (a) Staircase Scene, (b) Artificial Hill, (c) Rose Garden, (d) Botanical Garden, (e) Indoor Staircase Only \cs{and (f) Wet Grass(for stress test only)}. These datasets cover diverse terrain elevations, and each environment contains distinct stair structures for evaluating the $z$-axis localization performance of different methods.}
...
\cs{As shown in Table~\ref{tab:ablation_center_selection}, disabling adaptive center selection activates all candidate centers and slightly reduces the mean fitting error from 0.0944 m to 0.0939 m. However, the RBF computation time increases from 16.88 ms to 19.03 ms. Thus, the proposed center selection mainly improves online efficiency while maintaining comparable terrain approximation accuracy.}
...
\cs{As shown in Table~\ref{tab:ablation_gpu_acceleration}, the GPU and CPU implementations achieve comparable approximation accuracy. However, GPU acceleration reduces the total RBF computation time from 4506.92 ms to 22.17 ms, making online terrain reconstruction feasible.}
...
\cs{As shown in Table~\ref{tab:terrain_observation_stress}, all settings maintain a valid-frame ratio of 100\%. Although sparsification reduces the average number of active centers, the full-cloud validation errors remain close to the full-observation setting. This indicates that the RBF terrain approximation remains stable under moderate sparsification and partial field-of-view reduction, provided that sufficient local terrain support is available.}
...
\begin{figure*}[!htpb]
    \centering
    \includegraphics[width=0.95\textwidth]{figure/traj/new_stair_drift.png}
...
\yzl{We first examine the indoor staircase-only sequence as a stress test for abrupt elevation changes. As shown in Fig.~\ref{fig:stair_drift}, RBF-LIO reconstructs a structurally consistent staircase map. In contrast, the other methods exhibit different degrees of ghosting, layering, or staircase-surface distortion, indicating reduced robustness when repeated height transitions dominate the scene. This result directly illustrates the benefit of explicitly incorporating terrain geometry into the odometry optimization.}

\yzl{We further evaluate the mapping results on the outdoor sequences in Fig.~\ref{fig:garden-results}. The improvements are indeed most visible along the vertical dimension.}
\begin{figure}[!htbp] 
  \centering
...
\end{figure}
\yzl{In Rose Garden, where sparse features and abrupt height changes occur simultaneously, Fast-LIO2 suffers from map layering, while Wheel-Legged SLAM incorrectly fits the local plane and fails near the staircase region. In Botanical Garden, Leg-KILO shows noticeable drift on complex terrain. These failure cases are consistent with the limitations of scan-matching-only methods and local planar contact models. In contrast, RBF-LIO continuously uses the RBF terrain manifold to regularize the height estimate, resulting in more globally consistent maps.}

\yzl{Finally, Fig.~\ref{fig:traj} compares the trajectories on Rose Garden. This sequence is selected because it contains sparse features, grass slopes, and short staircases. Fast-LIO2 and Wheel-Legged SLAM diverge, while ROLO-SLAM produces a large pose error; therefore, these three trajectories are not shown. Leg-KILO and GLIM still exhibit z-axis drift, whereas RBF-LIO remains closest to the RTK reference.}
\begin{figure}[!htpb]
    \centering
...
\yzl{Next, we quantitatively compare RBF-LIO with the baselines using Absolute Trajectory Error (ATE) and Relative Trajectory Error (RTE), as reported in Tables~\ref{tab:ape} and~\ref{tab:rpe}. The values in parentheses denote the corresponding z-axis errors, which directly reflect the effect of the proposed manifold constraint on vertical localization. The best results are highlighted in bold.}
...
\yzl{Both GLIM and RBF-LIO successfully estimate poses on all evaluated sequences. On feature-rich sequences with abrupt height changes, such as Staircase Scene 1, Staircase Scene 2, and Artificial Hill, RBF-LIO achieves overall ATE values comparable to the strongest baselines while maintaining lower or competitive z-axis errors. On more challenging sequences such as Rose Garden and Botanical Garden, the advantage of RBF-LIO becomes more evident, especially in suppressing accumulated vertical drift. RBF-LIO also outperforms ROLO-SLAM, which is explicitly designed to reduce vertical errors.}

\yzl{The improvement in RTE is less pronounced than that in ATE because RTE measures short-horizon local consistency, whereas ATE reflects accumulated trajectory-wide drift. Since the proposed manifold constraint mainly takes effect during height transitions, its benefit is more clearly reflected in ATE and z-ATE. Overall, the qualitative and quantitative results show that RBF-LIO improves robustness under abrupt elevation changes while remaining competitive on normal terrain.}
...
\cs{To evaluate RBF-LIO under slippery-surface and contact-uncertain conditions, we collect an additional wet-grass sequence after rain and compare the full method with Fast-LIO2, GLIM, and RBF-LIO without the terrain manifold constraint.}
...
\cs{This stress test also shows that contact-based constraints should be treated as soft residuals on slippery or deformable surfaces. When large manifold residuals, insufficient terrain support, abnormal RBF fitting errors, or sudden IMU changes are detected, the constraint can be down-weighted or disabled, allowing the estimator to fall back to LiDAR--IMU scan matching.}
...
\centering
\includegraphics[width=0.75\linewidth]{figure/robot_setting/m20.png}\\
\footnotesize (a) Robot platform
\end{minipage}
...
\centering
\includegraphics[width=\linewidth]{figure/traj/m20_stair_drift3.png}\\
\footnotesize (b) Side and zoomed-in mapping results. The zoomed-in maps are produced by (a) RBF-LIO, (b) Leg-KILO, and (c) Wheel-Legged SLAM. 
\end{minipage}

\caption{\cs{Qualitative cross-platform staircase stress test on the M20 four-wheel-legged robot. The front-facing LiDAR provides less favorable ground observations than the inverted Mid360 setup used on the Tron1 platform. In the zoomed-in comparison, RBF-LIO preserves a clearer staircase structure, Leg-KILO produces a blurred and thick map, and Wheel-Legged SLAM suffers from severe drift.}}
\label{fig:m20_staircase_map}
\end{figure}

\section{Conclusion}
\yzl{This paper presented RBF-LIO, a terrain-aware LiDAR--inertial odometry framework for legged-wheel robots. The method reconstructs an online RBF terrain manifold from LiDAR observations and incorporates it into scan matching as a soft wheel--terrain constraint. Adaptive center selection and GPU acceleration support online terrain approximation. Experiments on staircases, slopes, gardens, degraded terrain observations, wet grass, and an additional M20 staircase sequence show that RBF-LIO suppresses z-axis drift and maintains stable mapping under challenging terrain conditions. Future work will focus on stronger contact-reliability estimation and quantitative cross-platform validation with platform-specific kinematic models.
