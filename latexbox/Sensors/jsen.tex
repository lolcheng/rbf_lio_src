...
\thanks{All the authors are with School of Automation and Intelligent Sensing, Shanghai Jiao Tong University, Institute of Medical Robotics, Shanghai Jiao Tong University, and Key Laboratory of System Control and Information Processing, Ministry of Education, Shanghai 200240, China. Corresponding author: Han Zhang (e-mail: zhanghan\_tc@sjtu.edu.cn}}
...
\begin{itemize}
    ...
    \item \cs{We develop terrain-aware soft constraints that couple the online RBF terrain manifold with the wheel--terrain contact geometry and integrate them into the LIO scan-matching optimization. Instead of treating terrain reconstruction as a separate mapping output, the proposed formulation uses the reconstructed terrain as an active geometric regularizer for pose estimation, thereby suppressing vertical drift and improving odometry robustness on rough terrain.}
...
Building upon these insights, we develop a terrain-aware RBF-based LIO framework for legged-wheel robots operating on uneven and unstructured terrain. In contrast to contact- or flatness-based methods such as Leg-KILO and Wheel-Legged SLAM, we construct a continuous and differentiable terrain manifold rather than relying on local planar assumptions. Compared with elevation-map-based approaches, our terrain model can be directly differentiated and incorporated into gradient-based optimization, thereby enabling smoother and more consistent geometric corrections during iterative pose refinement. Compared with GP-based terrain modeling, our recursive RBF approximation is more amenable to real-time implementation. More specifically, the approximated terrain geometry is directly integrated into the LIO optimization as soft constraints, which helps mitigate z-axis pose drift under abrupt elevation changes.

\cs{The proposed method is related to several lines of prior work, but differs from them in how terrain geometry is represented, updated, and used in state estimation. General-purpose LIO methods such as LOAM, LIO-SAM, Fast-LIO2, and GLIM mainly rely on LiDAR geometric features and inertial propagation, but do not explicitly construct a terrain model that can constrain the robot height and attitude. Contact- or kinematics-aided methods such as Leg-KILO and Wheel-Legged SLAM exploit the robot morphology, but their terrain information is typically introduced through local flatness assumptions, discrete contact constraints, or kinematic consistency, rather than through a continuously differentiable terrain function. Elevation-map-based approaches can encode terrain height, but the grid representation makes the resulting gradients resolution-dependent and less suitable for direct smooth optimization. GP-based terrain models provide continuous surfaces, but their computational cost is less favorable for online LIO when measurements accumulate.}
...
\end{equation}
where $\mathcal{F}^{e}_{k}$ and $\mathcal{F}^{p}_{k}$ denote the extracted edge and planar feature sets, respectively. Moreover, $d_e(\cdot)$ and $d_p(\cdot)$ denote the corresponding point-to-line and point-to-plane residuals. This formulation is effective when sufficient and well-distributed LiDAR geometric features are available. However, for a legged-wheel robot moving on stairs, slopes, grass, and stone roads, the vertical direction can be weakly constrained by scan matching alone. Abrupt body motion over uneven terrain may also degrade inertial propagation, which further leads to accumulated drift along the $z$-axis.
...
\end{equation}
where $\lambda_{M}$ is the weight of the manifold constraint. Equation~\eqref{eq:problem_total_cost} highlights the role of the terrain model: it is not merely a mapping output, but an active geometric regularizer inside the LIO optimization.

In practice, the problem is solved online in two coupled steps. Given the current pose estimate and the local terrain point cloud, the RBF terrain model is first updated to obtain the smooth function $f(\mathbf{x};\mathbf{w})$. Then, the resulting terrain manifold is queried at the wheel contact locations and introduced into scan matching through the residual $\mathbf{r}^{j}_{M}$. The next section presents the RBF-based terrain approximation, while Sec.~\ref{LIO_MC} derives the manifold residual and its integration into the LIO optimization.
...
\end{equation}
Therefore, the reported terrain-approximation error measures the consistency between the reconstructed RBF terrain surface and the LiDAR-observed terrain points in the same coordinate frame. It should be noted that this error is not computed from an external survey-grade terrain model; instead, it is evaluated from the LiDAR terrain observations used for online terrain reconstruction.}

\cs{For the parameter study of $\lambda$, we collect all valid errors $e^{i}_k$ over the Rose Garden sequence and compare their distributions under different ridge penalties. Since LiDAR measurements may contain outliers caused by water-puddle reflections, dynamic objects, or local non-terrain returns, we exclude the top 1\% largest errors when computing the statistics. As shown in Fig.~\ref{fig:boxplot}, $\lambda = 0.001$ leads to larger approximation errors because the regression becomes more sensitive to noise and numerical ill-conditioning, while $\lambda = 0.1$ also increases the error due to excessive smoothing. Therefore, we choose $\lambda = 0.01$, which is used in all subsequent experiments.}
...
\cs{As shown in Table~\ref{tab:ablation_center_selection}, compared with the full version, the average number of active centers increases from 883.8 to 920.9, while the active ratio increases from 96.0\% to 100.0\%. Since more centers are involved in the fitting process, the mean approximation error is only slightly reduced from 0.0944 m to 0.0939 m. However, this small accuracy change is obtained at the cost of higher computation. In particular, the average RBF computation time increases from 16.88 ms to 19.03 ms. Equivalently, the proposed adaptive center selection reduces the RBF computation time by about 11.3\% while maintaining comparable terrain approximation accuracy.}

\cs{These results show that the adaptive center selection strategy mainly improves computational efficiency rather than directly improving fitting accuracy. This behavior is reasonable because the Rose Garden sequence contains relatively dense terrain observations, and most candidate centers are already supported by nearby LiDAR points. Nevertheless, the proposed strategy removes unsupported or weakly supported centers, avoids unnecessary kernel evaluations, and keeps the online RBF terrain approximation more efficient. Therefore, adaptive center selection is useful for maintaining real-time performance while preserving the quality of the reconstructed terrain surface.}
...
\cs{As shown in Table~\ref{tab:ablation_gpu_acceleration}, the GPU and CPU implementations achieve comparable terrain approximation accuracy. The mean / 95\% errors are 0.1176 m / 0.4103 m for the GPU version and 0.1161 m / 0.4057 m for the CPU version. This indicates that GPU acceleration does not change the RBF approximation result in a meaningful way, but mainly affects computational efficiency.}

\cs{In terms of runtime, the GPU implementation significantly reduces the computational cost of all RBF-related operations. The kernel-matrix construction time decreases from 98.67 ms to 2.08 ms, the weight-solving time decreases from 4339.21 ms to 15.53 ms, and the total RBF computation time decreases from 4506.92 ms to 22.17 ms. This result demonstrates that GPU acceleration is essential for maintaining online terrain reconstruction in the proposed RBF-LIO framework.}

\subsection{Robustness to Degraded Terrain Observations}
\label{subsec:terrain_observation_stress}

\cs{To further evaluate the robustness of the proposed terrain representation under reduced ground observations, we conduct a controlled stress test on the Rose Garden sequence. This experiment is motivated by the concern that the proposed method may depend on dense terrain observations from the inverted LiDAR configuration. We simulate degraded terrain observations by artificially sparsifying or occluding the terrain point cloud before RBF fitting. In this way, the input to the RBF terrain approximation is degraded, while the remaining system settings are kept unchanged.}

\cs{We evaluate four settings. The first setting uses the original terrain observations and is denoted as ``full''. The second and third settings randomly retain only 50\% and 25\% of the terrain points, respectively. The fourth setting removes terrain points within a forward-facing angular sector of $60^\circ$ in the robot frame, which simulates partial occlusion or a reduced ground-viewing field of view. In all cases, the RBF model is fitted using the degraded terrain observations. To avoid reporting only training-set fitting errors, we additionally evaluate the reconstructed RBF terrain surface on the original full terrain point cloud of the same frame. Therefore, the reported validation error measures whether the terrain surface reconstructed from degraded observations remains consistent with the full local terrain observations.}

\begin{table}[!t]
\centering
\caption{\cs{Robustness test under degraded terrain observations on the Rose Garden sequence. The training error is computed on the degraded observations used for RBF fitting, while the full-cloud validation error is computed on the original full terrain point cloud of the same frame. Error values denote mean / 95\% vertical residuals in meters.}}
...
\begin{tabular}{|c|c|c|c|c|c|}
\hline
Setting & Kept ratio & Valid frames & Active centers & Train error & Full-val. error \\
\hline
Full & 100.0\% & 100.0\% & 744.6 & 0.0910 / 0.3367 & 1.5147 / 4.3010 \\

Sparse 50\% & 50.0\% & 100.0\% & 691.2 & 0.0653 / 0.2647 & 1.5492 / 4.3473 \\

Sparse 25\% & 25.0\% & 100.0\% & 616.9 & 0.0386 / 0.1782 & 1.4944 / 4.2071 \\

Sector occ. & 92.7\% & 100.0\% & 658.0 & 0.0932 / 0.3329 & 1.5026 / 4.2180 \\
\hline
\end{tabular}
\renewcommand{\arraystretch}{1.0}
\end{table}

\cs{As shown in Table~\ref{tab:terrain_observation_stress}, all tested settings maintain a valid-frame ratio of 100\%, which indicates that the RBF terrain update remains numerically stable under the considered observation degradation. When only 50\% and 25\% of the terrain points are retained, the average number of active RBF centers decreases from 744.6 to 691.2 and 616.9, respectively. This confirms that the degradation effectively reduces the amount of terrain support used for RBF fitting. Nevertheless, the full-cloud validation errors remain close to that of the full-observation setting. In particular, the mean / 95\% validation errors are 1.5147 m / 4.3010 m for the full setting, 1.5492 m / 4.3473 m for the 50\% sparse setting, and 1.4944 m / 4.2071 m for the 25\% sparse setting. These results suggest that, on the Rose Garden sequence, the proposed RBF terrain approximation is not overly sensitive to moderate random sparsification of ground observations.}

\cs{The sector-occlusion setting also maintains a comparable validation error, with a mean / 95\% error of 1.5026 m / 4.2180 m. Since the actual retained ratio is 92.7\%, this setting corresponds to partial local occlusion rather than extreme terrain loss. The result indicates that the proposed terrain representation can tolerate moderate field-of-view reduction or local occlusion as long as sufficient terrain points remain around the local region of interest. Meanwhile, the decrease in active centers under the sparse and occluded settings also reduces the RBF computation time. Overall, this stress test provides additional evidence that the proposed RBF terrain approximation can remain stable under reduced terrain observations, while also clarifying that sufficient local terrain support is still required for reliable manifold construction.}
...
\cs{To make the comparison protocol explicit, Table~\ref{tab:baseline_inputs} summarizes the sensor measurements and additional geometric information used by each method. We do not modify LiDAR--IMU-only baselines to use joint measurements or terrain contact information, because doing so would change their original algorithmic assumptions and require designing new factors that are outside the scope of those methods. Therefore, Fast-LIO2, GLIM, and ROLO-SLAM are used as representative general-purpose odometry baselines, while Leg-KILO and Wheel-Legged SLAM are used as more closely related kinematic/contact-aware baselines. In particular, the comparison with Leg-KILO and Wheel-Legged SLAM is closer to a matched-information setting, since these methods are also allowed to use joint-sensor measurements or robot kinematics. Under this setting, the main difference is how the additional robot-terrain information is modeled and incorporated into the odometry optimization.}
...
In addition to the input-information summary in Table~\ref{tab:baseline_inputs}, we keep the evaluation protocol and implementation settings as consistent as possible across all methods:
\begin{itemize}
...
\cs{The proposed RBF-LIO framework improves vertical localization by coupling the online RBF terrain manifold with wheel--terrain contact geometry. Nevertheless, the method relies on several physical and geometric assumptions. First, the wheels are assumed to maintain stable contact with the terrain during normal locomotion. Second, each wheel is assumed to have a unique effective contact point at each time step. Third, the local terrain around the robot is assumed to be representable as a single-valued continuous height function $p_z=f(p_x,p_y)$. These assumptions are reasonable for many legged-wheel navigation scenarios, such as slopes, stone roads, grass surfaces, and staircases with moderate step sizes, but they may be violated in more challenging cases.}

\cs{Potential failure cases include contact uncertainty, insufficient terrain observations, and terrain-model mismatch. Contact uncertainty may occur when the wheel slips, temporarily loses contact, or experiences strong vibration after impact. In such cases, the wheel--terrain manifold residual may no longer represent a reliable physical constraint. Insufficient terrain observations may occur under severe occlusion, unfavorable LiDAR mounting, or sparse ground returns, which can make the RBF surface poorly supported around the contact region. Terrain-model mismatch may occur near sharp stair edges, dense grass, curbs, vertical structures, overhanging surfaces, or multi-layer environments, where the local geometry may be non-smooth or not well represented by a single-valued height function. Therefore, the proposed terrain manifold constraint should be regarded as a soft regularizer rather than a hard constraint.}

\cs{In practice, these failure cases can be handled by monitoring both geometric and inertial consistency. For example, the number of terrain points supporting each contact region and the active RBF center ratio can be used to evaluate whether the terrain manifold is sufficiently constrained. The RBF fitting residual and the full-cloud validation residual can indicate whether the reconstructed terrain surface is consistent with the observed point cloud. The manifold residual in (\ref{eq:residual_manifold_left_wheel}) can further measure whether the optimized wheel contact geometry agrees with the terrain model. In addition, IMU acceleration spikes and abnormal angular velocities can indicate impacts or strong vibrations, while inconsistencies between leg kinematics and LiDAR--IMU motion can suggest wheel slip or contact loss.}

\cs{When these indicators become unreliable, the manifold constraint can be adaptively down-weighted or temporarily disabled. Since the proposed constraint is introduced as a soft residual in the scan-matching objective, disabling it does not invalidate the LIO front-end; instead, the system degrades to the original LiDAR--IMU scan-matching formulation. Once sufficient terrain support and contact consistency are recovered, the terrain constraint can be reactivated. The degraded-observation stress test in Sec.~\ref{subsec:terrain_observation_stress} also supports this interpretation: the RBF terrain approximation remains stable under moderate sparsification and partial occlusion, but reliable manifold construction still requires sufficient local terrain support.}

\cs{Finally, the current experiments are conducted on one legged-wheel robot platform with an inverted Mid360 LiDAR. Although the proposed formulation does not mathematically require this specific mounting configuration, the performance of the terrain manifold constraint depends on the availability of local terrain observations around the wheel contact region. Therefore, extending the evaluation to different LiDAR mounting configurations, additional robot platforms, and more severe non-smooth or non-single-valued terrain structures remains an important direction for future work.}
...
& 3.3049 (0.3987) & 1.8872 (1.0807) \\
Rose Garden
& \textbf{0.5824} (0.1043) & \textbf{1.0972} (\textbf{0.2628})
& /\ & /\
