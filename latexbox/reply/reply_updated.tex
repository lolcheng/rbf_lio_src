...
\begin{itemize}
    \item We added a new \textit{Preliminaries and Problem Formulation} section to clarify the terrain-aware odometry problem, the limitations of grid-based elevation maps and local planar constraints, and the role of the RBF terrain manifold in pose optimization.
    \item We clarified the novelty of the proposed method in the Introduction and Related Work sections, emphasizing that the key contribution is the coupling of online RBF terrain modeling, wheel--terrain contact geometry, and LIO scan-matching optimization.
    ...
    \item We added ablation studies for the terrain manifold constraint, adaptive RBF center selection, and GPU acceleration, together with a regularization-parameter study.
    \item We added stress tests under degraded terrain observations and wet grass to evaluate reduced ground observations, partial occlusion, and slippery/contact-uncertain conditions.
    \item We added a qualitative cross-platform staircase stress test on a DeepRobotics Lynx M20 four-wheel-legged robot with a front-facing LiDAR. The revised manuscript compares RBF-LIO with Leg-KILO and Wheel-Legged SLAM in this setting.
    \item We clarified baseline fairness by listing the sensor measurements and additional geometric information used by each method, and we added clearer limitation analysis on contact reliability, terrain support, and single-valued terrain assumptions.
\end{itemize}
...
\Response{Thank you for this valuable comment. We agree that the original manuscript did not provide enough evidence for reduced terrain observations and cross-platform deployment. In the revised manuscript, we added three complementary stress tests.

First, we added \textbf{Sec. VII-D: Robustness to Degraded Terrain Observations}. In this controlled experiment, the terrain point cloud is degraded before RBF fitting by random sparsification and sector occlusion. The RBF surface is fitted using the degraded observations and then evaluated on the original full terrain point cloud of the same frame. The results show a 100\% valid-frame ratio under all tested settings, and the full-cloud validation errors remain close to the full-observation setting. This indicates that the proposed RBF terrain approximation remains stable under moderate sparsification and partial field-of-view reduction, as long as sufficient local terrain support is available.

Second, we added \textbf{Sec. VII-F: Wet-Grass Stress Test} to evaluate the behavior of the method under slippery-surface and contact-uncertain conditions. The wet-grass sequence is collected after rain using the same sensor configuration as the other outdoor sequences. Compared with the ablated RBF-LIO without the terrain manifold constraint, the full RBF-LIO reduces the $z$-ATE from 0.1850 m to 0.1442 m, corresponding to a relative reduction of about 22.1\%, while maintaining a similar RTE. This result suggests that the soft terrain manifold improves vertical consistency without degrading short-term odometry stability under moderate contact uncertainty.

Third, we added \textbf{Sec. VII-G: Cross-Platform Staircase Stress Test} on a DeepRobotics Lynx M20 four-wheel-legged robot. This platform uses a front-facing LiDAR configuration, which provides less favorable and less uniform ground observations than the inverted Mid360 setup on the Tron1 platform. The selected half-floor staircase segment contains 183 poses over 36.40 s, with a trajectory length of 12.03 m and an elevation range of 2.23 m. We further compare RBF-LIO with Leg-KILO and Wheel-Legged SLAM on this M20 staircase sequence. The qualitative results show that RBF-LIO preserves a clearer staircase structure, Leg-KILO produces a blurred and thick map, and Wheel-Legged SLAM suffers from severe drift. This provides additional qualitative evidence under a different robot platform, a different LiDAR mounting configuration, and a non-smooth staircase terrain.

We also explicitly state that the M20 experiment is not intended as a strict quantitative benchmark because the platform morphology, LiDAR mounting, ground-observation density, and wheel--terrain contact geometry differ from those of Tron1. A complete quantitative cross-platform validation still requires ground-truth trajectories and platform-specific contact modeling.}
...
\Response{Thank you for this important suggestion. We have revised the experimental discussion to make the assumptions and potential failure cases more explicit. In the revised wet-grass and cross-platform stress-test sections, we clarify that the proposed wheel--terrain manifold constraint relies on reliable wheel--terrain contact, sufficient local terrain support, and a terrain surface that can be locally represented as a single-valued height function.

We further discuss representative failure cases, including slippery or deformable grass, severe wheel slip, temporary loss of contact, strong vibration, sharp stair edges, insufficient terrain support, and multi-layer or non-single-valued structures. These cases may weaken the contact assumption or make the RBF terrain query unreliable near the wheel contact region.

To address this issue, we emphasize that the terrain manifold term is introduced as a soft residual rather than a hard constraint. Large manifold residuals, insufficient terrain support around the contact region, abnormal RBF fitting errors, and sudden IMU acceleration or angular-velocity changes can be used as reliability cues. When such cues indicate unreliable contact or terrain support, the manifold residual can be down-weighted or rejected, allowing the estimator to rely on the LiDAR--IMU scan-matching objective. We have added this discussion to make the operating assumptions and limitations of the proposed method clearer and to explain how the system can handle contact-uncertain or failure-prone cases.}
